...
export async function GET() {
  // The 2nd type param tells the client what each row actually looks like,
  // instead of letting it infer (and fail on) the joined `brand` shape.
  const { data, error } = await supabaseServer
    .from("ev_prices")
    .select<typeof SELECT_WITH_BRAND, EvPriceWithBrand>(SELECT_WITH_BRAND)
  ...
  // Sort by brand name client-side since Supabase can't order by a joined
  // column directly in a single .order() call here.
