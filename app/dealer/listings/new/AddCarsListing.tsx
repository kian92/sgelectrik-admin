...
import { ArrowLeft, Car, CheckCircle2, ImageIcon, Loader2 } from "lucide-react";
...
export function AddListingForm({ dealerSlug, dealerName }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${form.brand} ${form.model} is now live on SGElectrik.`,
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              {dealerName} · Published immediately to SGElectrik
            </p>
          </div>
        </div>
      </div>
        ...
        {form.brand && form.model && (
          <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            <div className="text-sm text-emerald-800">
              ...
              {form.priceMin && (
                <> · from S${Number(form.priceMin).toLocaleString()}</>
              )}
              {form.rangeKm && <> · {form.rangeKm} km range</>} — will appear
              live on SGElectrik after publishing.
            </div>
          </div>
        )}
