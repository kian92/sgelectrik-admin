...
export default function DealerDashboard() {
  ...
  const rentalCompanySlug = profile?.rental_company?.slug;
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="space-y-2 mb-4">
                ...
                {profile.rental_company?.fleet.map((rentalCar) => (
                  <div
                    key={`rental-${rentalCar.id}`}
                    className="flex items-center justify-between py-1.5 px-3 rounded-xl bg-slate-50"
                  >
                    <div className="flex items-center gap-2">
                      <Car className="h-4 w-4 text-slate-400" />
                    ...
                    <Link
                      href={`${process.env.NEXT_PUBLIC_USER_URL}/ev-rentals/${rentalCompanySlug}/${slugify(rentalCar.model)}`}
                      target="_blank"
                      className="text-xs text-emerald-600 hover:underline"
                    >
                      View
