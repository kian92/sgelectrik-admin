...
export default function RentalsAdminClient({
  initialCompanies,
  initialPage,
}: Props) {
  // Local state so deletes reflect immediately without a full page reload
  ...
  async function handleDelete(id: number, name: string) {
    ...
    try {
      ...
      alert("Failed to delete rental company. Please try again.");
    } finally {
      setDeletingId(null);
    }
  }
      ...
      <div className="mb-8 flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            All rental companies on SGElectrik
          </p>
        </div>
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="py-20 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1 mb-4">
              Add the first rental company to get started.
            </p>
            <Link href="/admin/rentals/new">
              <Button className="gap-2">
                <Plus className="h-4 w-4" /> Add company
