...
export default function AdminEvPricesPage() {
  ...
  async function saveRow(row: DraftRow) {
    ...
    try {
      if (row.isNew) {
        const res = await fetch("/api/admin/ev-prices", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            brandId: row.brandId,
        ...
        const res = await fetch(`/api/admin/ev-prices/${row.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            brandId: row.brandId,
    ...
    <div className="max-w-screen-xl mx-auto">
      <div className="border-b border-border/60 bg-muted/20">
        <div className="container mx-auto max-w-screen-xl px-4 md:px-8 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-foreground">EV Price List</h1>
            <p className="text-sm text-muted-foreground">
              Manage brands, then add or edit models under each one.
            </p>
          </div>
        </div>
      </div>
        ...
        <section className="rounded-2xl border border-border/60 p-5">
          ...
          <form onSubmit={addBrand} className="flex items-center gap-2 mb-4">
            <input
              ...
              placeholder="New brand name (e.g. Tesla)"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-muted/40 text-left">
                      ...
                      <th className="px-4 py-3 font-semibold text-foreground text-right w-44">
                        Price From (S$)
                      </th>
